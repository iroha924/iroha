...
- **Report everything you actually found, and label how sure you are.** Do not pre-filter to your most confident findings — a separate pass filters, and a suppressed real defect costs more than a labelled uncertain one. What is forbidden is manufacturing a concern with no triggering scenario, or asserting one exists when you have not found it.
- **Reproduce before you report.** You have `Bash`: write the throwaway test, run it, paste the output. A reproduced finding is acted on directly; an argued one has to be re-derived by a separate validation pass. Say which findings you reproduced and which are reasoning only, and delete any probe files you created.
- **Check whether an existing test already claims to cover it.** When you find a defect in code that has a test aimed at it, say why the test does not catch it — a test that passes vacuously (seeding data that filters out before it reaches the assertion, comparing two reads of unchanged data) is a second finding, and the more durable one.

## Output

Report findings using the same severity framing as the project's other review tooling: file, line, the concrete failure scenario (exact input/state/ordering that triggers it), the observable consequence (wrong output, crash, silent data loss/corruption), and whether you reproduced it. If you find nothing, say so explicitly — do not manufacture a finding to seem thorough. Do not fix anything yourself; this is a read-only adversarial pass.
