...
- With those two, **nothing injects a runtime `<style>`, so `style-src 'self'` holds with no nonce** — matching `contracts/dashboard-api.md` §9 literally. A per-process CSP-nonce round-trip was built and then **removed as dead YAGNI code** once `disableStyleElements` + color-less charts covered every injector; do not re-add it speculatively.
- **If a future component MUST inject a `<style>`** (a config-colored chart, a Dialog scroll-lock, a new library), the e2e gate below fails first. Only then add a per-process nonce back — server side in `security.ts` (`style-src 'self' 'nonce-…'`, cached on `globalThis` so a bundled duplicate can't mint two nonces) + a `<meta name="csp-nonce">` injected by `static.ts` + Base UI `CSPProvider nonce` — **and record it in this file and in `security.ts`'s comment**, because adding a `'nonce-…'` to `style-src` is a deviation from §9's `style-src 'self'`.

## Banned: sonner — and any library that injects an un-nonced `<style>`

- **sonner (toasts) is fundamentally incompatible with this CSP.** It injects its CSS via `head.appendChild(<style>)` with **no nonce support** (verified in its dist), so every toast style is refused under `style-src 'self' 'nonce-…'`. **Do not reintroduce `sonner`.**
- **Toasts themselves are fine — the ban is on sonner, not on the pattern.** `components/ui/toast.tsx` is shadcn's `base-nova` toast over `@base-ui/react/toast`, and it clears the CSP for two reasons: every style is a Tailwind class or a CSS custom property (the stacking/swipe animation is `--toast-index`/`--toast-swipe-movement-*`, which Base UI writes through CSSOM), and the `toast/` module contains no style-element injection at all — only `ScrollAreaRoot` and `SelectPopup` inject, and both are already disabled by `disableStyleElements`. It also needs no new dependency: `@base-ui/react` was already installed, so there is no catalog entry to pin. `<Toaster closeLabel={…}/>` is mounted once in `App.tsx`, outside `<main>`, so a confirmation survives the navigation that raised it.
- `renderWithProviders` (`test-utils.tsx`) mounts `Toaster` for the same reason: `toast.add` renders nothing without it, so a page test asserting a save confirmation would pass vacuously.
