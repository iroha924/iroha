...
- **The schemas in `packages/domain/src/schemas/*.ts` are mirrors of the repo-root `schemas/*.schema.json` (JSON Schema)** (there used to be an identical copy under `docs/product/schemas/` as well, but ID-029 consolidated the repo root as the single source of truth). Fixing only one and leaving the other alone makes the runtime validation (Zod) and the contract documentation (JSON Schema) silently diverge. When adding or changing a schema, update both (the Zod `.ts` and `schemas/*.schema.json`) in the same commit. The `test:contracts` that would automatically verify this synchronization is not currently implemented in any package (planned for a future WP alongside the MCP contract test — `implementation-plan.md`), so **maintaining the synchronization manually is mandatory for now**.
