...
iroha is a local-first Engineering Memory Graph for Claude Code and Codex (TypeScript/Node 24, pnpm workspaces, libSQL, Zod, MCP server, Hook adapters, a local Hono API + React dashboard). This file is the entry point for any coding agent working in this repo, including the two hosted reviewers that read a pull request here (Codex, and Gemini Code Assist via `.gemini/styleguide.md`) — it does not assume you have already read `CLAUDE.md` or `.claude/rules/*.md`, since those are loaded automatically only inside Claude Code sessions. Read the files this document points to; do not guess at their content.
