...
- **ローカルファースト、既定でプライベート。** クラウドアカウントも、テレメトリも、ホスティング型バックエンドもありません。生のプロンプトやトランスクリプトが知識ベースに書き込まれることはありません。外部への通信は任意かつオプトインの 2 つだけです。埋め込み生成（Voyage）と GitHub forge 連携で、どちらも自分の鍵で有効にするまでは動きません。
- **設定ゼロで動く検索。** 全文検索とグラフ検索は最初から使えます（日本語などの CJK テキストにも対応）。設定でセマンティック検索を有効にして埋め込み用の鍵を足せば使えますが、そのどちらかが無ければ全文検索のままで、失敗はしません。
...
`iroha dashboard` は、ローカルの単一オリジンアプリを 1 つの loopback ポートで配信し、ブラウザにワンタイムの launch token を渡します。ここで知識の候補をレビューし、各項目の出どころと関連を確認して、承認または却下します。承認が行われるのはダッシュボードだけです。これは人間の操作であり、エージェントの手は届きません。ダッシュボードは loopback にのみバインドし、マシンの外には何も公開しません。
...
| 環境変数 | 用途 | 必須 | 未設定のとき |
|---|---|---|---|
| `VOYAGE_API_KEY` | Voyage によるセマンティック（ベクトル）検索の埋め込み | いいえ | 全文検索とグラフ検索のみのままになる |
| `GITHUB_TOKEN` | GitHub forge 連携（Pull Request・レビュー）の同期 | いいえ | forge 連携が無効になるだけ |

どちらの機能も **`config.yaml` では既定で無効**で、鍵はその半分にすぎません。セマンティック検索には `search.embedding.enabled: true`（と `VOYAGE_API_KEY`）を、GitHub 連携には `forge.enabled: true`（と `GITHUB_TOKEN`）を設定します。
...
5. **ローカルの libSQL インデックスが検索を支えます**（全文は常に、セマンティックは埋め込みを有効にして鍵を設定したとき）。このインデックスは使い捨てで、いつでも `.iroha/` から再構築できます。承認済み知識の唯一の情報源になることはありません。

## よくある質問

**何かがマシンの外に出ますか？** 既定では出ません。iroha はローカルファーストで、クラウドアカウントもテレメトリもありません。自分の鍵でオプトインしたときだけネットワークに接続する機能が 2 つあります。セマンティック検索の埋め込み（Voyage）と GitHub forge 連携で、どちらも有効にするまでは動きません。
