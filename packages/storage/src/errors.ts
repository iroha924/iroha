...
/**
 * Matches contracts/database.md §3: `PRAGMA busy_timeout = 2500`
 ...
 * (contracts/mcp.md §4). `cause` keeps the original `LibsqlError` for local
 * diagnostics, but `message`/`details` never include raw SQL, bound
 * argument values, or filesystem paths — contracts/mcp.md §4 forbids
