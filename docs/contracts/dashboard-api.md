...
`GET /api/v1/knowledge` query parameters: `cursor`, `limit`, `offset`, `status` (repeatable; `approved`|`imported`|`superseded`|`archived`, default `approved`), `type` (repeatable; one of the seven knowledge `entity_type`s). `imported` is the repository's own instruction documents (canonical.md §14), which the list shows alongside approved knowledge and the status badge tells apart. Values outside these sets are ignored, and `type` never widens beyond the knowledge set. The response carries `total` alongside `items` and `nextCursor`, so the page can number itself.
