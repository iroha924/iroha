...
既存 `.iroha/config.yaml` の `api_key_env` は、schema が `strictObject` であるため放置すると parse に失敗する。**読み込み時に除去したうえで、`iroha init` がファイルを書き戻して消す**。読み込み時の除去だけでは commit されたファイルは変わらず、それを読んだ teammate が今は無効な環境変数を設定して黙って何も起きない、という状態が残るためである。ユーザーに手作業を要求せず、git diff には 1 行の削除として現れる。

0.5.x はこの key に `UPPER_SNAKE_CASE` 制約を課しており、値として鍵そのものを貼った場合は loud に失敗していた。key を drop する移行はその検査を失わせるので、`iroha doctor` が「環境変数名に見えない値が残っている」ことを warning として報告する — 既に commit 済みであれば git 履歴に鍵があるため、rotate を促す必要がある。

## 14. Privacy and security

**canonical（`.iroha/`）・local DB・log・診断出力**への保存禁止:
...
この禁止は上記 4 つの保存先を対象とする。ADR-018 の machine-scoped credential file はその範囲外であり、そこが provider の API key を平文で持つ唯一の場所である（file 0600 / dir 0700、値は API response にも log にも error にも載らない）。
...
| ADR-018 | provider credentialは環境変数をやめ`~/.config/iroha/credentials.json`（machine-scoped、0600）からオンデマンドで読む | Accepted |
